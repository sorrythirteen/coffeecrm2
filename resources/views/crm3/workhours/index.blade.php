...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <form action="{{ route('crm3.workhours.index') }}" method="GET" class="d-flex" style="max-width: 300px;">
        <input 
            type="search" 
            name="search" 
    ...
    <a href="{{ route('crm3.workhours.create') }}" class="btn btn-primary shadow-sm px-4 py-2 fw-semibold rounded-pill">
        Добавить запись
    </a>
</div>
  ...
  <tbody>
    ...
    <tr class="align-middle">
      ...
      <td class="text-center" style="white-space: nowrap;">
        <a href="{{ route('crm3.workhours.show', $entry) }}" class="btn btn-sm btn-info me-1 shadow-sm rounded-pill">
          <i class="bi bi-eye"></i> Просмотр
        </a>
        <a href="{{ route('crm3.workhours.edit', $entry) }}" class="btn btn-sm btn-warning text-white me-1 shadow-sm rounded-pill">
          <i class="bi bi-pencil"></i> Редактировать
        </a>
        <form action="{{ route('crm3.workhours.destroy', $entry) }}" method="POST" style="display:inline-block" onsubmit="return confirm('Удалить запись?')">
          @csrf
          @method('DELETE')
