...
@section('title', 'Добавить рабочее время CRM 2')

@section('content')
<h2 class="mb-4">Добавить запись рабочего времени</h2>

<a href="{{ route('crm2.workhours.index') }}" class="btn btn-secondary mb-4">
    ← Вернуться к списку
</a>

<form action="{{ route('crm2.workhours.store') }}" method="POST" lang="ru">
    @csrf
