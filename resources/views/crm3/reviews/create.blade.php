...
<form action="{{ route('crm3.reviews.store') }}" method="POST" class="shadow-sm p-4 bg-white rounded">
    @csrf
    ...
    <div class="mb-3">
        ...
        <select name="rating" id="rating" class="form-select @error('rating') is-invalid @enderror" required>
            ...
            @for($i = 1; $i <= 5; $i++)
                <option value="{{ $i }}" @selected(old('rating') == $i)>{{ $i }} звезд</option>
            @endfor
        </select>
        @error('rating')
            <div class="invalid-feedback">{{ $message }}</div>
    ...
    <!-- <div class="mb-3">
        ...
        <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
            <option value="pending" @selected(old('status') == 'pending')>Ожидает модерации</option>
            <option value="approved" @selected(old('status') == 'approved')>Одобрен</option>
            <option value="rejected" @selected(old('status') == 'rejected')>Отклонен</option>
        </select>
        @error('status')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
