...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    <form action="{{ route('crm3.menu.index') }}" method="GET" class="d-flex" style="max-width: 300px;">
        <input 
            type="search" 
            name="search" 
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('crm3.dashboard') }}" class="btn btn-primary shadow-sm px-4 py-2 fw-semibold rounded-pill">
            <i class="bi bi-arrow-left-circle me-1"></i> Дашборд
        </a>
        <a href="{{ route('crm3.menu.create') }}" class="btn btn-primary shadow-sm px-4 py-2 fw-semibold rounded-pill">
            Добавить позицию
        </a>
    </div>
</div>
  ...
  <tbody>
    ...
    <tr class="align-middle">
      ...
      <td class="text-center" style="white-space: nowrap;">
        <a href="{{ route('crm3.menu.show', $item) }}" class="btn btn-sm btn-info me-1 shadow-sm rounded-pill" title="Просмотр">
          <i class="bi bi-eye"></i> Просмотр
        </a>
        <a href="{{ route('crm3.menu.edit', $item) }}" class="btn btn-sm btn-warning text-white me-1 shadow-sm rounded-pill" title="Редактировать">
          <i class="bi bi-pencil"></i> Редактировать
        </a>
        <form action="{{ route('crm3.menu.destroy', $item) }}" method="POST" style="display:inline-block" onsubmit="return confirm('Удалить позицию меню?')">
          @csrf
          @method('DELETE')
